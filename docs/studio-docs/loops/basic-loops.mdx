...
```html
{#loop your_loop_name as item}
  <!-- Render fields from the custom query, e.g. title, featured image, etc. -->
{/loop}
```
...
{#loop mainQuery as item}
  <!-- Render the current posts from the main query -->
{/loop}
```
...
For more details on how `mainQuery` works (including default arguments and loop arguments), see [Main Query Loops](/loops/main-query).
...
Since our loop is saying `{#loop loopName as item}` we need to use `item` as stem for our dynamic data keys.
...
When a loop has no items, it renders nothing by default.

If you want to display a message when this happens, you can add a second loop that only runs when the loop data returns no results:

```html
<div>
  {#loop posts as item}
    <!-- Renders as normal, but when the loop returns no results, then nothing is rendered -->
    <p>{item.title}</p>
  {/loop}

  {#loop posts.length().equal(0, [1], []) as item}
    <!-- Only renders when the loop returns no results -->
    <p>No results</p>
  {/loop}
</div>
```

This is currently a workaround. A more official way to handle empty loop states will be added in the future.
...
```html
{#loop recentPosts as post, index}
