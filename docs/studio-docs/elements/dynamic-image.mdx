...
:::info
Etch has native support for IMG HTML, but this is separate from Etch's Dynamic Image element. If you want to use a `<img>` tag in Etch, you're always free to do that. The Dynamic Image Element is an alternative special element designed specifically to images by their ID.
:::
...
In the Attributes Panel, with the Dynamic Image Element selected, you can choose a source (media library) or manually enter a Media ID.
