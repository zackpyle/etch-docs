  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
          ...
          <VersionBadge version="1.0.0-alpha-5" style={{marginBottom: 0}}/>
      </td>
...
The `.pluck()` method extracts the value at a given property path from each element in an array of objects and returns those values as a new array. As expected, paths can be nested using dot notation (e.g., `"author.name"`) to get a nested property's values. Similar to array_column() in PHP or pluck() in Laravel.
...
**After `.pluck()`, you can now do it in a single condition:**
```
