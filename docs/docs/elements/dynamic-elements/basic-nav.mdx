...
<div class="video-embed">
  <iframe
    src="https://iframe.mediadelivery.net/embed/472639/2a6462b8-8d9d-4677-aa4c-7f225136b40a?autoplay=false&loop=false&muted=false&preload=true&responsive=true"
    loading="lazy"
...
:where(
	.etch-nav__link[aria-current='page'],
	.etch-nav__button[aria-current='page']
) {
...
9. From the component editor, add a "Text" prop. Make sure the prop key is `navAriaLabel`.
10. Save the component and exit the component editor.
11. Choose the Basic Nav loop once again from the Loop selector in the component attributes panel. This will explicitly set the loop so you're not relying on the default value.
12. Set a descriptive aria-label for the nav in the component attributes panel. This is important for accessibility.
...
6. From the component editor, add a "Text" prop. Make sure the prop key is `ariaLabel`.
7. Save the component and exit the component editor.
8. Set a descriptive aria-label for the burger in the component attributes panel. This is important for accessibility.
...
**Examples** of using the `ariaCurrentPage` option:
- Enable the `aria-current` attribute on the current page:
- Disable the homepage link from being marked as the current page:
```js
instantiateNavClass({
...
const burgerInstance = new EtchBurgerScript({
	...
	onToggle: (isOpen) => {
		document.body.classList.toggle('my-class', isOpen); // toggle a class on the body
	},
	onClose: myOwnFunction // our own function that will do something that we need.
});
